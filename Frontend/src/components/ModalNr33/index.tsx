...
export function ModalNr33({ isOpen, onRequestClose, controll, handleFinishOrder  }: ModalOrderProps){
  const [id, setId] = useState(controll[0].id)
  const [name, setName] = useState(controll[0].name)
  const [descNr33, setDescNr33] = useState(controll[0].descNr33)
...
async function handatualizar(){
 ...
 <button
    ...
    <div className={styles.titulo}>Marcação de NR33 </div>
...
value={descNr33}
onChange={(a) => setDescNr33(a.target.value)}
></textarea>
</label>
           <div className={styles.linha}></div>
